function [net, stats] = cnn_train_jutsu(net, imdb, getBatch, varargin)
...
% @JoshuaSandvick Set up parameters for confusion matrix
opts.mode = mode;
opts.epoch = state.epoch;
...
%{ Increment timestep for ADAM algorithm and gather b1 & b2
net.layers{1}.t = net.layers{1}.t + 1;
t = net.layers{1}.t;
b1 = net.layers{1}.b1;
b2 = net.layers{1}.b2;
epsilon = net.layers{1}.epsilon;
%}
...
for l=numel(net.layers):-1:1
    for j=1:numel(res(l).dzdw)
        ...
        if j == 3 && strcmp(net.layers{l}.type, 'bnorm')
            ...
            % ADAM algorithm
            dzdw = res(l).dzdw{j}; %+ randn(size(res(l).dzdw{j}), 'single') * sqrt(0.01 / (1 + t) ^ 0.55);
            net.layers{l}.meanavg{j} = b1 * net.layers{l}.meanavg{j} ...
                + (1 - b1) * dzdw / batchSize;
            net.layers{l}.varavg{j} = b2 * net.layers{l}.varavg{j} ...
                + (1 - b2) * dzdw .* dzdw / (batchSize ^ 2);
            adjLRate = state.learningRate * sqrt((1 - b2 ^ t)) / (1 - b1 ^ t);
            net.layers{l}.weights{j} = net.layers{l}.weights{j} - ...
                adjLRate * net.layers{l}.meanavg{j} ./ (sqrt(net.layers{l}.varavg{j}) + epsilon);
        end
